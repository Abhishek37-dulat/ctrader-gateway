...
export class OAuthService {
  ...
  async exchangeCodeAndStore(
    ...
    await this.store.saveTokens(
      ...
      // tokens are ~30 days; keep slightly longer or same
  ...
  async refreshAndStore(userId: string): Promise<OAuthTokens> {
    ...
    // Some providers may omit refresh_token on refresh; keep old if missing
    await this.store.saveTokens(
      userId,
      {
        accessToken: tokens.accessToken,
  ...
  private async exchangeCode(code: string): Promise<OAuthTokens> {
    ...
    body.set("code", code);
    body.set("client_id", env.ctrader.clientId);
    body.set("client_secret", env.ctrader.clientSecret);
    body.set("redirect_uri", env.ctrader.redirectUri);
  ...
  private normalizeTokens(raw: any): OAuthTokens {
    // API sometimes returns both camelCase + snake_case
